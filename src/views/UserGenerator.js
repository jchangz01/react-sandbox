...
export default class SearchView extends React.Component {
    ...
    saveUser = () => {
        if (this.state.saved_users.slice(-1)[0] !== this.state.generated_user) {
            console.log(this.state.saved_users)
            this.setState({ saved_users: [...this.state.saved_users, this.state.generated_user] });
        }
    }

    filter = (phrase, filter) => phrase.indexOf(filter) !== -1;
    handleFilterStrChange = (newFilterStr) => { this.setState({filter_str : newFilterStr}); };
    ...
    render() {
        ...
        const SavedUsersDisplay = SavedUsersFiltered.reverse().slice(0, max_users_shown).map((user) => {
            return (
                <div style={{flexBasis: 100/users_per_rows + '%'}} key={user.username} >
        ...
        return (
            <div className="usergen-content">
                ...
                <section>
                    <div className="usergen-sec-container">
                        ...
                        <div className="usergen-btn-container">
                            <button onClick={this.fetchUser} className="usergen-btn">New User</button>
                            <button onClick={this.saveUser} className="usergen-btn">Save User</button>
                        </div>
                    </div>
                </section>
                <section>
                    <div className="usergen-sec-container" style={{border:'none'}}> 
                        ...
                        <InputFilter 
                            ...
                            results_count={SavedUsersDisplay.length}
                        />
                        <small>Display per row: </small>
                        <select value={users_per_rows} onChange={(e) => this.setState({ users_per_rows: parseInt(e.target.value)})}>
                ...
                <section style={{marginBottom: "40px"}}>
                    <div className="usergen-user-cards-content">
                        {SavedUsersDisplay}
                    </div>
                    { 
                        SavedUsersDisplay.length > max_users_shown ? 
                        <p className="usergen-show-more" onClick={this.setMaxUserShown}>Show more users</p> :
                        null
                    }   
                </section>
            </div>
